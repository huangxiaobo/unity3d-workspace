...
public class Timer
{
	...
	public double GetFps ()
	{
		if (fpstimes > 0) {
			...
			fpstimes = 0;
			totalfps = 0;
			if (fps == double.PositiveInfinity)
				return 0;
			return fps;
		} else
			return 0;
	...
	{
		if (_MilisecondsElapsed > _Milisecconds || _Milisecconds == 0)
			return true;
	...
	{
		AddMethod (-1, _Action2, null, true);
	}

	public void AddMethod (Func<bool> func, Action _Action2)
	{
		AddMethod (-1, _Action2, func, true);
	}

	public void AddMethod (int _Miliseconds, Action _Action2)
	{
		AddMethod (_Miliseconds, _Action2, null, true);
	}
